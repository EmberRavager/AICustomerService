...
class TenantCreateRequest(BaseModel):
    ...
    model_config_: Optional[Dict[str, Any]] = Field(
        None, alias="model_config", description="模型配置"
    )
...
class TenantXianyuConfigRequest(BaseModel):
    """闲鱼账号配置"""

    cookies: Optional[str] = Field(None, description="闲鱼 Cookies")
    seller_name: Optional[str] = Field(None, description="闲鱼账号名称")
    enabled: Optional[bool] = Field(None, description="是否启用闲鱼账号")
...
# 统一响应格式
def create_response(
    data: Any = None, message: str = "操作成功", success: bool = True, code: int = 200
) -> Dict[str, Any]:
    """
    创建统一的API响应格式
    """
    ...
    try:
        ...
        return JSONResponse(
            ...
            content=create_response(
                success=False, code=503, message=f"系统异常: {str(e)}"
            ),
        )
...
async def send_message(
    ...
    try:
        logger.info(
            f"收到聊天请求: session_id={request.session_id}, user_id={request.user_id}"
        )
...
async def send_message_stream(
    ...
    async def generate_response() -> AsyncGenerator[str, None]:
        try:
            logger.info(
                f"收到流式聊天请求: session_id={request.session_id}, user_id={request.user_id}"
            )
...
async def get_chat_history(
    ...
    try:
        logger.info(
            f"获取聊天历史: session_id={session_id}, limit={limit}, offset={offset}"
        )

        history = await chat_service.get_chat_history(
            session_id, limit or 50, offset or 0, tenant_id=tenant_id
        )

        return create_response(
            data=history, message=f"成功获取 {len(history)} 条历史记录"
        )
...
async def get_chat_sessions(
    ...
    try:
        ...
        sessions = await tenant_memory_service.get_user_sessions(
            user_id, limit or 20, offset or 0
        )

        return create_response(
            data=sessions, message=f"成功获取 {len(sessions)} 个会话"
        )
...
async def create_chat_session(
    ...
    try:
        ...
        return create_response(data=session, message="会话创建成功")
...
async def get_chat_settings(chat_service: ChatService = Depends(get_chat_service)):
    """
    获取聊天设置
    """
    try:
        ...
        return create_response(data=settings_data, message="获取设置成功")
...
async def update_chat_settings(
    settings_update: ChatSettings, chat_service: ChatService = Depends(get_chat_service)
):
    """
    更新聊天设置
    """
    try:
        ...
        return create_response(data=updated_settings, message="设置更新成功")
...
async def search_knowledge(
    ...
    search_type: Optional[str] = Query(
        "hybrid", description="搜索类型: vector, keyword, hybrid"
    ),
    ...
    try:
        ...
        results = await tenant_knowledge_service.search_knowledge(
            query=query, limit=limit or 10, search_type=search_type or "hybrid"
        )

        return create_response(
            data=results, message=f"搜索完成，找到 {len(results)} 条相关内容"
        )
...
async def get_item_info(
    ...
    try:
        ...
        return create_response(data=item_context, message="获取商品信息成功")
...
async def order_status_callback(
    ...
    is_paid = status_text in paid_signals or any(
        signal in raw_status_text for signal in paid_signals
    )
...
async def add_knowledge(
    ...
    try:
        ...
        return create_response(data=result, message="知识库内容添加成功")
...
async def update_knowledge(
    ...
    try:
        ...
        tenant_knowledge_service = KnowledgeService(tenant_id)
        result = await tenant_knowledge_service.update_knowledge(
            knowledge_id, knowledge
        )

        return create_response(data=result, message="知识库内容更新成功")
...
async def list_knowledge(
    ...
    try:
        logger.info(
            f"获取知识库列表: category={category}, limit={limit}, offset={offset}"
        )

        tenant_knowledge_service = KnowledgeService(tenant_id)
        knowledge_list = await tenant_knowledge_service.list_knowledge(
            category=category, limit=limit or 20, offset=offset or 0
        )

        return create_response(
            data=knowledge_list, message=f"成功获取 {len(knowledge_list)} 条知识库内容"
        )
...
async def get_knowledge_categories(
    ...
    try:
        ...
        return create_response(data=categories, message="获取分类列表成功")
...
@tenant_router.post("", response_model=Dict[str, Any])
async def create_tenant(
    ...
    try:
        ...
        if payload.model_config_:
            updates["model_config"] = payload.model_config_
        if updates:
...
@tenant_router.get("/{tenant_id}", response_model=Dict[str, Any])
async def get_tenant_config(
    tenant_id: str, tenant_service: TenantService = Depends(get_tenant_service)
):
    """获取租户配置"""
    try:
...
@tenant_router.put("/{tenant_id}", response_model=Dict[str, Any])
...
@tenant_router.get("/{tenant_id}/xianyu", response_model=Dict[str, Any])
async def get_tenant_xianyu_config(
    tenant_id: str, tenant_service: TenantService = Depends(get_tenant_service)
):
    """获取租户闲鱼账号配置"""
    try:
        config = tenant_service.get_tenant_config(tenant_id)
        xianyu = config.get("xianyu", {})
        cookies = xianyu.get("cookies")
        masked = ""
        if cookies:
            masked = f"{cookies[:6]}...{cookies[-4:]}"
        data = {
            "enabled": xianyu.get("enabled", False),
            "seller_name": xianyu.get("seller_name"),
            "has_cookies": bool(cookies),
            "cookies_masked": masked,
        }
        return create_response(data=data, message="获取闲鱼账号配置成功")
    except Exception as e:
        logger.error(f"获取闲鱼账号配置失败: {e}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="获取闲鱼账号配置失败")


@tenant_router.put("/{tenant_id}/xianyu", response_model=Dict[str, Any])
async def update_tenant_xianyu_config(
    tenant_id: str,
    payload: TenantXianyuConfigRequest,
    tenant_service: TenantService = Depends(get_tenant_service),
):
    """更新租户闲鱼账号配置"""
    try:
        updates: Dict[str, Any] = {}
        xianyu_updates: Dict[str, Any] = {}
        if payload.cookies is not None:
            xianyu_updates["cookies"] = payload.cookies.strip()
        if payload.seller_name is not None:
            xianyu_updates["seller_name"] = payload.seller_name.strip()
        if payload.enabled is not None:
            xianyu_updates["enabled"] = payload.enabled
        if xianyu_updates:
            updates["xianyu"] = xianyu_updates
        config = tenant_service.update_tenant_config(tenant_id, updates)
        xianyu = config.get("xianyu", {})
        cookies = xianyu.get("cookies")
        masked = ""
        ...
        return create_response(data=data, message="闲鱼账号配置已更新")
    except Exception as e:
        logger.error(f"更新闲鱼账号配置失败: {e}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="更新闲鱼账号配置失败")


@tenant_router.get("", response_model=Dict[str, Any])
async def list_tenants(tenant_service: TenantService = Depends(get_tenant_service)):
    """获取租户列表"""
    try:
...
@tenant_router.post("/reset-key", response_model=Dict[str, Any])
...
async def batch_add_knowledge(
    ...
    try:
        ...
        return create_response(
            data=results, message=f"批量添加完成，成功添加 {len(results)} 条内容"
        )
