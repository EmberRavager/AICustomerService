...
const WidgetPage: React.FC = () => {
  ...
  return (
    <div className="widget-page">
      <div className="widget-header">在线客服</div>

      {(itemInfoLoading || itemInfo || itemInfoError) && (
        <div className="widget-item-info">
          {itemInfoLoading && <span>正在加载商品信息...</span>}
          {itemInfoError && <span className="widget-item-error">{itemInfoError}</span>}
          {itemInfo && (
            <div className="widget-item-content">
              <div className="widget-item-title">{itemInfo.title || '未知商品'}</div>
      ...
      <div className="widget-messages">
        {messages.length === 0 && (
          <div className="widget-empty">请输入问题开始对话</div>
        )}
        {messages.map((msg) => (
