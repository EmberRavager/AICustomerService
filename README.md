...
- **多机器人隔离**: 每个机器人（租户）独立模型配置与知识库
- **账号接入**: 支持为机器人绑定闲鱼账号 Cookie
...
系统采用「机器人为主体」的后台结构，功能入口清晰分区：

- **概览**: 入口指引与核心操作
- **机器人管理**: 创建机器人、绑定 Key、闲鱼账号配置
- **测试聊天**: 绑定机器人后进行对话验证
- **知识库**: 检索/列表/新增/批量导入
- **策略配置**: 自动发货/意图路由/风控/人工接管
- **模型管理**: 模型切换/测试
- **聊天设置**: 提示词/记忆窗口/历史长度
- **运维工具**: 健康检查/订单回调测试
...
进入「测试聊天」页面后，请先绑定 `X-Tenant-Key`，再开始对话。

### 闲鱼账号绑定

可在「机器人管理」中为指定机器人绑定闲鱼账号 Cookie，用于商品信息拉取与后续消息接入。

管理员接口示例：

```bash
GET /api/tenants/{tenant_id}/xianyu

PUT /api/tenants/{tenant_id}/xianyu
{
  "cookies": "your_xianyu_cookies",
